# GMI Cloud H100 ISB-1 quickstart
> Manual GMI Cloud path only. No Slurm, enroot, squash files, or GitHub runner labels.
## What you need
- GMI Cloud H100: 1x node is enough; 2x lets you run code-8k/chat-32k on one node and code-131k on the other
- SSH access, read-only HF token, ~30 minutes
- This branch checked out on the box
## Provision + SSH
Create 1-2 H100 instances in GMI Cloud, note the public IP(s), then:
```bash
ssh ubuntu@<gmi-ip>
```
## Install once
...
- Model from upstream H100/H200 Qwen trace-replay wiring: `Qwen/Qwen3.5-397B-A17B-FP8`
- Pinned vLLM image: `vllm/vllm-openai:v0.18.0`
- Corpus alias: `hf_wchen22--isb1-cc-traces`
- Manual offload mapping here: `on` = `--cpu-offload-gb 20`; `off` = no offload flag; `noprefix` = offload off + `--no-enable-prefix-caching`
## Prep once
```bash
...
start_server () {
  ...
  [[ "$offload" == 'on' ]] && offload_flag='--cpu-offload-gb 20'
  docker rm -f qwen35-h100 >/dev/null 2>&1 || true
  docker run -d --name qwen35-h100 --gpus all --ipc=host --network host \
  ...
  docker logs --tail=200 qwen35-h100 || true
  return 1
}
stop_server () { docker rm -f qwen35-h100 >/dev/null 2>&1 || true; }
```
## Per-cell settings
```bash
...
- Tarball:
```bash
tar -C "$ARTIFACT_ROOT" -czf "$PWD/isb1-gmi-h100-$(basename "$ARTIFACT_ROOT").tgz" .
```
